...
public class AresEasyScenario : PredefinedScenario
{
    ...
    static AresEasyScenario()
    {
        ...
        for (int i = 0; i < aresLevelCount; i++)
        {
        ...
        levels[5].AddEnemies(EnemyType.SCARAB_DRONE, 30, 1.0f, defaultWaypoint);
        ...
        levels[1].AddEnemies(EnemyType.SCRAP_TITAN, 5, 0.5f, defaultWaypoint);

        levels[2].AddEnemies(EnemyType.SCRAP_TITAN, 1, 1.0f, WaypointsConstants.WaypointRoute.ARES_SPECIAL);

        levels[3].AddEnemies(EnemyType.HEAVY_BEHEMOTH, 2, 1.0f, defaultWaypoint);

        levels[4].AddEnemies(EnemyType.SCRAP_TITAN, 1, 1.0f, defaultWaypoint);
    }
